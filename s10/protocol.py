...
class MessageBus:
    ...
    def read_inbox(self, name: str) -> list:
        ...
        if not inbox_path.exists():
            return []
        messages = []
        # strip() 清理最前和最后的换行符和空格
        # split() 不保留每一行内部的空白 ['Name:', 'Tom', 'Age:', '20']
        # splitlines() 保留空格， 严格按行分割 ['Name: Tom', 'Age: 20']
        for line in inbox_path.read_text().strip().splitlines():
            if line:
                messages.append(json.loads(line))
        # 读完消息后写空重置 （可能存在send到了一个消息但是没写入message直接情况）
    ...
    def spawn(self, name: str, role: str, prompt: str) -> str:
        ...
        if member:
            if member["status"] not in ("idle", "shutdown"):
                return f"Error: {name} is currentlt {member['status']}"
            member["status"] = "working"
            member["role"] = role
        else:
            member = {"name": name, "role": role, "status": "working"}
            self.config["member"].append(member)
        # 添加新的member子智能体
        self._save_config()
        thread = threading.Thread(
            target = self._teammate_loop,
            args = (name, role, prompt),
            daemon = True, # 主进程结束即结束，服务主进程
        )
...
def run_subagent(prompt: str) -> str:
    ...
    for _ in range(30):
        ...
        for tool_call in tool_calls:
            ...
            # tools return
            print(output[:200])
            sub_messages.append({"role": "tool", "tool_use_id": tool_call.id, "name": tool_call.function.name, "content": str(output)})
